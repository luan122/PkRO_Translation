...
bool mapif_homunculus_save(struct s_homunculus* hd)
{
	...
	{// new homunculus
		if( SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `homunculus` "
			"(`char_id`, `class`,`name`,`level`,`exp`,`intimacy`,`hunger`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`,`skill_point`, `rename_flag`, `vaporize`, `lvlevo1`, `lvlevo2`, `resto_str`, `resto_agi`, `resto_vit`, `resto_dex`, `resto_int_`, `resto_luk`) "
			"VALUES ('%d', '%d', '%s', '%d', '%u', '%u', '%d', '%d', %d, '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')",
			hd->char_id, hd->class_, esc_name, hd->level, hd->exp, hd->intimacy, hd->hunger, hd->str, hd->agi, hd->vit, hd->int_, hd->dex, hd->luk,
			hd->hp, hd->max_hp, hd->sp, hd->max_sp, hd->skillpts, hd->rename_flag, hd->vaporize, hd->lvlevo1, hd->lvlevo2, hd->resto_str, hd->resto_agi, hd->resto_vit, hd->resto_dex, hd->resto_int_, hd->resto_luk) )
		{
	...
	{
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `homunculus` SET `char_id`='%d', `class`='%d',`name`='%s',`level`='%d',`exp`='%u',`intimacy`='%u',`hunger`='%d', `str`='%d', `agi`='%d', `vit`='%d', `int`='%d', `dex`='%d', `luk`='%d', `hp`='%d',`max_hp`='%d',`sp`='%d',`max_sp`='%d',`skill_point`='%d', `rename_flag`='%d', `vaporize`='%d', `lvlevo1`='%d', `lvlevo2`='%d', `resto_str`='%d', `resto_agi`='%d', `resto_vit`='%d', `resto_dex`='%d', `resto_int_`='%d', `resto_luk`='%d' WHERE `homun_id`='%d'",
			hd->char_id, hd->class_, esc_name, hd->level, hd->exp, hd->intimacy, hd->hunger, hd->str, hd->agi, hd->vit, hd->int_, hd->dex, hd->luk,
			hd->hp, hd->max_hp, hd->sp, hd->max_sp, hd->skillpts, hd->rename_flag, hd->vaporize, hd->lvlevo1, hd->lvlevo2, hd->resto_str, hd->resto_agi, hd->resto_vit, hd->resto_dex, hd->resto_int_, hd->resto_luk, hd->hom_id) )
		{
			Sql_ShowDebug(sql_handle);
			flag = false;
		}
...
bool mapif_homunculus_load(int homun_id, struct s_homunculus* hd)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `homun_id`,`char_id`,`class`,`name`,`level`,`exp`,`intimacy`,`hunger`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`,`skill_point`,`rename_flag`, `vaporize`, `lvlevo1`, `lvlevo2`, `resto_str`, `resto_agi`, `resto_vit`, `resto_dex`, `resto_int_`, `resto_luk` FROM `homunculus` WHERE `homun_id`='%u'", homun_id) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	Sql_GetData(sql_handle, 21, &data, NULL); hd->lvlevo1 = atoi(data);
	Sql_GetData(sql_handle, 22, &data, NULL); hd->lvlevo2 = atoi(data);
	Sql_GetData(sql_handle, 23, &data, NULL); hd->resto_str = atoi(data);
	Sql_GetData(sql_handle, 24, &data, NULL); hd->resto_agi = atoi(data);
	Sql_GetData(sql_handle, 25, &data, NULL); hd->resto_vit = atoi(data);
	Sql_GetData(sql_handle, 26, &data, NULL); hd->resto_dex = atoi(data);
	Sql_GetData(sql_handle, 27, &data, NULL); hd->resto_int_ = atoi(data);
	Sql_GetData(sql_handle, 28, &data, NULL); hd->resto_luk = atoi(data);
