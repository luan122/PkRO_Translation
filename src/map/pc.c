...
int pc_countitem(struct map_session_data *sd, int itemid)        //[Hold -brA]
{
	...
	if(!(it_tmp = itemdb_exists(itemid)))
		return 0;
	...
	for(i = 0; i < MAX_INVENTORY; i++)
		if(sd->status.inventory[i].nameid == id)
			amt += sd->status.inventory[i].amount;
...
int pc_additem(struct map_session_data *sd,struct item *item_data,int amount)
{
	...
	if( item_data->nameid > 8030 && item_data->nameid < 8182){
	for( i = 8031; i < 8182; i++ ){
	...
	if( amt2 >= 6 ){
		storage_storageadd(sd,item_data->nameid,1);
		return 7;
	}
	}
	sd->status.lock = 1;
	...
	if(sd->weight + w > sd->max_weight)
		if( item_data->nameid < 8030 && item_data->nameid > 8182 && item_data->nameid != 690)
			return 2;
...
int pc_dropitem(struct map_session_data *sd,int n,int amount)
{
	...
	if(sd->status.inventory[n].nameid == 690 || sd->status.inventory[n].nameid > 8030 && sd->status.inventory[n].nameid < 8182)
		return 0;
...
//	sc = status_get_sc(hd);
	...
	switch( nameid )
	{
		...
		if(hd){
			if(hom->evomoment == 1){	
				clif_displaymessage(sd->fd, "VocÍ n„o pode retornar seu pokemon enquanto ele evolue.");
				return 0;
			}	
		break;
		}	
		...
		case 8031: //Bulbassaur
		case 8032: //ivysaur
		case 8033: //venusaur
		case 8034: //Charmander
		case 8035: //Charmeleon
		case 8036: //Charizard
		case 8037: //Squirtle
		case 8038: //Wartortle
		case 8039: //Blastoise
		...
		case 8064: //vulpix
		case 8065: //ninetales
		case 8066: //nidoran femea
		case 8067: //wigglypuff
		case 8068: //zubat
		case 8069: //golbat
		case 8070: //oddish
		case 8071: //gloom
...
//				if(sd->status.hom_id <= 1987 && pc_isGM(sd) < 99){
//				clif_displaymessage(sd->fd, "Este pokemon precisa ser atualizado. Procure Algum ADM para realizar a troca.");
//				return 0;
//				}
				...
				//sd->status.skill[NORMAL_TACKLE].id = NORMAL_TACKLE;
				//sd->status.skill[NORMAL_TACKLE].lv = 10;
				//sd->status.skill[NORMAL_TACKLE].flag = 1;
				//clif_addskill(sd,NORMAL_TACKLE);	
	...
	break;
		...
		case 19029: //crystal
		if(status->def_ele ==  12 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		}
		break;

		case 19024: //cocoon
		if(status->def_ele ==  13 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19053: //fire
		if(status->def_ele ==  3 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19192: //water
		if(status->def_ele ==  1 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19031: //dark
		if(status->def_ele ==  8 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19039: //earth
		if(status->def_ele ==  11 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19046: //enigma
		if(status->def_ele ==  6 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19074: //heart
		if(status->def_ele ==  0 && hd->homunculusDB->emax.luk < 100  && hom->evomoment == 0 && hd->homunculus.class_ != 2633|| status->def_ele ==  4 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0&& hd->homunculus.class_ != 2633){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19084: //ice
		if(status->def_ele ==  14 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19093: //leaf
		if(status->def_ele ==  7 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19130: //punch
		if(status->def_ele ==  10 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19136: //rock
		if(status->def_ele ==  2 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19178: //thunder
		if(status->def_ele ==  9 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
		...
		case 19189: //venom
		if(status->def_ele ==  5 && hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		return 1;
		}else{	
		clif_displaymessage(sd->fd, "Seu pokemon n„o pode evoluir atravez desta pedra.");
		return 0;
...
int pc_gets_status_point(int level)
{
	if (battle_config.use_statpoint_table) //Use values from "db/statpoint.txt"
		return 0;//(statp[level+1] - statp[level]);
