...
//----------------Skills pkRO---------------
			...
			case FIGHT_DEMONKICK:			
				hitrate += 100 * hitrate * 5 * skill_lv / 50;
				break;				
...
//-----------------Skills pkRO--------------------
					...
					case FIGHT_DEMONKICK:					
						{
							...
							hd = BL_CAST(BL_HOM, src);
							hom = &hd->homunculus;
						if(target->type == BL_HOM || target->type == BL_MOB){
						var = ((sstatus->int_)/25);
						if(var < 1){
						var = 1;
						}
						var2 = (((100 - hom->level)*(hom->level))/800);
						if(var2 < 1){
						var2 = 1;
						}
						 skillratio = ((((300 +(((sstatus->int_)+(50*skill_lv))/3)*2)+(((100 - hom->level)*(hom->level))/5))/var)/var2)+(skill_lv ^2);
			//------------Ajuste Final------			 
						 skillratio += 0; 
			//------------------------------			 
						 }else{ 
			...
			case MOB_TACKLE:			
					{
							...
							md = BL_CAST(BL_MOB, src);						
						var = (sstatus->str)/3;
						var2 = (((100 - md->level)*(md->level))/900);
						if(var2 < 1){
						var2 = 1;
						}
						skillratio += (((30*skill_lv) + ((sstatus->str)/3))/var2)-var;
						skillratio -= ((sstatus->str)/7);
						skillratio -= 100;
						if(target->type != BL_HOM)
						 skillratio = (skillratio/10)*8;
						break;
					}
			...
			case NORMAL_TACKLE:
					{
						if(target->type == BL_HOM || target->type == BL_MOB){
							...
							hom = &hd->homunculus;	
						var = (sstatus->str)/3;
						var2 = (((100 - hom->level)*(hom->level))/900);
						if(var2 < 1){
						var2 = 1;
						}
						skillratio += (((30*skill_lv) + ((sstatus->str)/3))/var2)-var;
		...
		//-----------------------------------		
			...
			case MOB_THUNDERSHOCK:			
						...
						var2 = (((100 - md->level)*(md->level))/800);
						if(var2 < 1){
						var2 = 1;
						}
						 skillratio = (((300 +(((sstatus->int_)+(50*skill_lv))/3)*2)+(((100 - md->level)*(md->level))/5))/var)/var2;
...
//					break;
	...
	if(hd)
		wd.damage += (wd.damage/100)*(hd->homunculus.intimacy/100);
...
//						var2 = 1;
//						//}
//						 //skillratio = (((300 +(((sstatus->int_)+(50*skill_lv))/3)*2)+(((100 - hom->level)*(hom->level))/5))/var)/var2;
//						 skillratio = ((((300 +(((sstatus->int_)+(50*skill_lv))/3)*2)+(((100 - hom->level)*(hom->level / 10))/5))/var)/var2)+(skill_lv ^2);
//						 
//						 //backup   ((((300 +(((sstatus->int_)+(50*skill_lv))/3)*2)+(((100 - hom->level)*(hom->level))/5))/var)/var2)+(skill_lv ^2);
//			//------------Ajuste Final------			 
//						 skillratio += 900 - ((hom->level*10)/2);   //nao testado
//			//------------------------------
//						 if(target->type == BL_PC)
//						 skillratio /= 5;

						skillratio += 1300 + 20*skill_lv - (hom->level * 10);
...
//						 //backup   ((((300 +(((sstatus->int_)+(50*skill_lv))/3)*2)+(((100 - hom->level)*(hom->level))/5))/var)/var2)+(skill_lv ^2);
//			//------------Ajuste Final------			 
//						 skillratio += 1000 - ((hom->level*10)/2);   //nao testado
//			//------------------------------
//						 if(target->type == BL_PC)
//						 skillratio /= 5;
...
//						 skillratio /= 5;
						...
						skillratio += 1100 + 20*skill_lv - (hom->level * 10);
...
//						 skillratio /= 5;
						...
						skillratio += 900 + 20*skill_lv - (hom->level * 10);
...
//----------------------------------------------------------------------------------------------
	...
	if(hd){
		if(hd->homunculus.class_ == 2542 && skill_num == BUG_ABSORV)
			ad.damage -= ad.damage/4;
}

	if(hd)
		ad.damage += (ad.damage/100)*(hd->homunculus.intimacy/100);
...
// eAthena additions
	...
	{ "show_mob_info",                      &battle_config.show_mob_info,                   0,      0,      1|2|4|3,        },
