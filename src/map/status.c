...
//----------------------------------------------------------------------------------------
//Funcs do Storage.c para AddStorage 7∫ Pk
...
void initChangeTables(void)
{
	...
	set_sc( MO_STEELBODY         , SC_STEELBODY       , SI_BLANK       , SCB_NONE );
	//SI_STRIPARMOR
	set_sc( STONE_HARDEN         , SC_HARDEN          , SI_BLANK		 , SCB_DEF);
	set_sc( MOB_HARDEN		     , SC_HARDEN          , SI_BLANK       , SCB_DEF);
	set_sc( FIGHT_EXTREMEAGILITY , SC_EXTREMEAGILITY  , SI_BLANK  , SCB_ASPD|SCB_SPEED );
	set_sc( MOB_EXTREMEAGILITY   , SC_EXTREMEAGILITY  , SI_BLANK  , SCB_ASPD|SCB_SPEED );
	set_sc( FIGHT_RAGE			 , SC_RAGE			  , SI_BLANK		   , SCB_STR );
	set_sc( MOB_RAGE			 , SC_RAGE			  , SI_BLANK		   , SCB_STR );	
	set_sc( NORMAL_REST          , SC_REST            , SI_BLANK                 , SCB_NONE );
	set_sc( MOB_REST             , SC_REST            , SI_BLANK                 , SCB_NONE );		
	...
	set_sc( CASH_ASSUMPTIO            , SC_SHRINK          , SI_SHRINK          , SCB_NONE );
	set_sc( CASH_ASSUMPTIO      , SC_CLOSECONFINE2   , SI_CLOSECONFINE2   , SCB_NONE );
	set_sc( CASH_ASSUMPTIO      , SC_CLOSECONFINE    , SI_CLOSECONFINE    , SCB_FLEE );
	set_sc( CASH_ASSUMPTIO      , SC_SIGHTBLASTER    , SI_SIGHTBLASTER    , SCB_NONE );
	set_sc( CASH_ASSUMPTIO         , SC_WINKCHARM       , SI_WINKCHARM       , SCB_NONE );
	add_sc( CASH_ASSUMPTIO         , SC_STUN            );
	add_sc( CASH_ASSUMPTIO      , SC_ELEMENTALCHANGE );
	add_sc( CASH_ASSUMPTIO       , SC_ELEMENTALCHANGE );
	add_sc( CASH_ASSUMPTIO     , SC_ELEMENTALCHANGE );
	add_sc( CASH_ASSUMPTIO       , SC_ELEMENTALCHANGE );
...
//		return 0; //Cannot damage a bl not on a map, except when "charging" hp/sp
	...
	switch (target->type) {
		...
		case BL_HOM: flag = merc_hom_dead((TBL_HOM*)target,1); break;
...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int flag)
{
	...
	// BA_DISSONANCE tambÈm remove o hide do alvo
...
int status_calc_homunculus_(struct homun_data *hd, bool first)
{
	...
	skill = status->vit;//hom->level/10 + status->vit/5;
	status->def = (skill/4)+1-skill;// cap_value(skill, 0, 99);
	...
	status->rhw.atk = ((1*(status->str)) + (hom->level))+20;//status->dex;
	status->rhw.atk2 = ((2*(status->str)) + (hom->level))+20;//status->str + hom->level;
...
static signed char status_calc_def(struct block_list *bl, struct status_change *sc, int def)
{
	...
	//--------------
	if(sc->data[SC_HARDEN]) //&& !battle_config.renewal_setting&20
		//return 50;
		def = 30;
	//--------------		
...
//---------------------------------------------------
		if( sc->data[SC_EXTREMEAGILITY] && max < sc->data[SC_EXTREMEAGILITY]->val2 )
			max = sc->data[SC_EXTREMEAGILITY]->val2;
//----------------------------------------------------	
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	switch(type)
	{
		...
		case SC_REST:  sc->opt1 = OPT1_SLEEP;     break;
		//OPT2
...
int status_change_end_(struct block_list* bl, enum sc_type type, int tid, const char* file, int line)
{
	...
	switch(type){
		...
		case SC_CAUGHTPB1:		
		{
		if(sd->status.trycaught [1] != 0){
		char message[128];
		int amt = 0;
		
		if( pc_isdead(sd) )
		{
			clif_displaymessage(sd->fd, "Sua Pokebola se perdeu.");
			break;
		}

		for( i = 8031; i < 8181; i++ ){
			amt += pc_countitem(sd, i);
		}
		amt += 1;
		if((hd = sd->hd) == NULL ){
			amt -= 1;
		}
		rate = sd->status.trycaught3 [1];
		sprintf (message, msg_txt(719), sd->status.name, sd->status.trycaught2 [1], (float)rate/100);
		intif_broadcast(message,strlen(message)+1,0x10);
		rate = sd->status.trycaught [1]; // 5530;

		sprintf(variavel, "caught_%d", rate);
		if(pc_readglobalreg(sd,variavel) == 0){
			idpk = pc_readglobalreg(sd,"caught");
			pc_setglobalreg(sd, "caught", idpk+1);
		}
		pc_setglobalreg(sd, variavel, 1);


//		sd->status.pokes [rate - 2500] = 1;
//		sd->status.caught = 0;
//		for(i = 1; i < 152; i++){
//			sd->status.caught = sd->status.caught + sd->status.pokes [i];
//		}	
		...
		tmp_item.nameid = rate+5530;
		tmp_item.identify = 1;
		tmp_item.refine=5;
		tmp_item.card[0] = CARD0_HUN;
		tmp_item.card[3] = rate;
		if(amt>5){
			clif_displaymessage(sd->fd, "VoÁÍ ja possui 6 Pokemons. Seu Pokemon foi enviado ao Centro Pokemon.");
			if((flag = storage_additem(sd,&tmp_item,1))) {
			...
			if(sd->status.chance == 1){
				char message[128];
				rate = sd->status.trycaught3 [1];
				sprintf (message, "VocÍ teve %0.02f%s de chance de capturar este pokemon.", (float)rate/100,"%");
		...
		case SC_CAUGHTPB2:
			...
			status_set_viewdata(&dstmd->bl, dstmd->class_);
			clif_changeoption(&dstmd->bl);
			unit_remove_map(&dstmd->bl,CLR_OUTSIGHT);
			status_kill(&dstmd->bl);			
			}
		break;	
		...
		rate = sd->status.trycaught3 [2];
		sprintf (message, msg_txt(719), sd->status.name, sd->status.trycaught2 [2], (float)rate/100);
		intif_broadcast(message,strlen(message)+1,0x10);
		rate = sd->status.trycaught [2]; // 5530;
//		sd->status.pokes [rate - 2500] = 1;
//		sd->status.caught = 0;
//		for(i = 1; i < 152; i++){
//			sd->status.caught = sd->status.caught + sd->status.pokes [i];
//		}	
		...
		rate = sd->status.trycaught3 [3];
		sprintf (message, msg_txt(719), sd->status.name, sd->status.trycaught2 [3], (float)rate/100);
		intif_broadcast(message,strlen(message)+1,0x10);
		rate = sd->status.trycaught [3]; // 5530;
//		sd->status.pokes [rate - 2500] = 1;
//		sd->status.caught = 0;
//		for(i = 1; i < 152; i++){
//			sd->status.caught = sd->status.caught + sd->status.pokes [i];
//		}	
		...
		rate = sd->status.trycaught3 [4];
		sprintf (message, msg_txt(719), sd->status.name, sd->status.trycaught2 [4], (float)rate/100);
		intif_broadcast(message,strlen(message)+1,0x10);
		rate = sd->status.trycaught [4]; // 5530;
//		sd->status.pokes [rate - 2500] = 1;
//		sd->status.caught = 0;
//		for(i = 1; i < 152; i++){
//			sd->status.caught = sd->status.caught + sd->status.pokes [i];
//		}	
		...
		rate = sd->status.trycaught3 [5];
		sprintf (message, msg_txt(719), sd->status.name, sd->status.trycaught2 [5], (float)rate/100);
		intif_broadcast(message,strlen(message)+1,0x10);
		rate = sd->status.trycaught [5]; // 5530;
//		sd->status.pokes [rate - 2500] = 1;
//		sd->status.caught = 0;
//		for(i = 1; i < 152; i++){
//			sd->status.caught = sd->status.caught + sd->status.pokes [i];
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_REST:
		...
		{ // Recovers 1% HP/SP every 2 seconds.
			...
			status_heal(bl, (status->max_hp)/35, 0, 2);
