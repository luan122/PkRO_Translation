...
//--------------------------------------------------------------------------------------	
		...
		if( sd->hd && !check_distance_bl(&sd->bl, &sd->hd->bl, 14) )		
		{
...
//		You can't move if you can't attack neither.
	...
	if(sd && sd->status.continuos == 0)
		sd->status.continuos2 = 0;

	if(sd){
	if(ud->state.attack_continue && sd->status.continuos2 == 0){
		ud->attacktimer = add_timer(ud->attackabletime,unit_attack_timer,src->id,0);
	}else{
		return 0;
	}
	...
	if(sd && sd->status.continuos == 1)
		sd->status.continuos2 = 1;
