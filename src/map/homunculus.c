...
int merc_hom_delete(struct homun_data *hd, int emote)
{
	...
	hd->homunculus.intimacy = 1;//0;
	// Send homunculus_dead to client
	hd->homunculus.hp = 1;//0;
	clif_hominfo(sd, hd, 0);
	return 0;//unit_remove_map(&hd->bl,CLR_OUTSIGHT);
}
	...
	for(i=0;i < MAX_SKILL_TREE && (id = hskill_tree[c][i].id) > 0;i++)
	{
		...
		if(!battle_config.skillfree)
		{
			for(j=0;j<MAX_PC_SKILL_REQUIRE;j++)
			{
				...
				if(hd->homunculus.level < hskill_tree[c][i].need[j].id)
				{
					f=0;
					break;
				}	
		...
		if (f)
			...
			hd->homunculus.hskill[id-HM_SKILLBASE].lv = hskill_tree[c][i].need[0].lv;
	}
	return 0;
}
...
int merc_hom_levelup(struct homun_data *hd)
{
	...
	struct npc_data *nd2 = npc_name2id("homlastup");
	struct map_session_data *sd;
	char output[256] ;
...
//		return 0;
	...
	sd->status.homlastuplvl = hom->level;
	sd->status.homlastupid = hd->homunculus.hom_id;
...
//		hom->skillpts++ ;	//1 skillpoint each 3 base level
	...
	hom->resto_str += growth_str;
	hom->resto_agi += growth_agi;
	hom->resto_vit += growth_vit;
	hom->resto_dex += growth_dex;
	hom->resto_int_ += growth_int;
	hom->resto_luk += growth_luk;
	...
	if (hom->level >= hd->homunculusDB->emax.luk && hom->evomoment == 0){
		//merc_hom_evolution(hd);
		clif_displaymessage(sd->fd, "Seu Pokemon está evoluindo!");
...
int merc_hom_decrease_intimacy(struct homun_data * hd, unsigned int value)
{
	if (hd->homunculus.intimacy > value)
		...
		hd->homunculus.intimacy = 1;

	if(hd->homunculus.intimacy < 1)
		hd->homunculus.intimacy = 1;
...
int merc_create_homunculus_request(struct map_session_data *sd, int class_)
{
	...
	//Initial data
	strncpy(homun.name, homunculus_db[i].name, NAME_LENGTH);
	homun.class_ = class_;
	homun.level = 1;
	homun.hunger = 32; //32%
