...
bool mob_ksprotected (struct block_list *src, struct block_list *target)
{
	...
	if(md->class_ == 2643 || md->class_ == 2649 || md->class_ == 2642 || md->class_ == 2644 || md->class_ == 2645 || md->class_ == 2646 || md->class_ == 2650 || md->class_ == 2651 || md->class_ == 1324)
		return false;
	...
	do {
		...
		if( DIFF_TICK(sd->ks_floodprotect_tick, tick) <= 0 )
		{
			...
			if(sd->status.ksmod == 1){
			if((sd) == NULL || (hd = sd->hd) == NULL ){
				return -1;
			}else{
			merc_hom_vaporize(sd,1);
			pc_randomwarp(sd,CLR_TELEPORT);
			//unit_free(&hd->bl,CLR_DEAD);
			sd->status.ksmod = 0;
			return 0;
			}
			}
		...
		// Message to Owner
		if( DIFF_TICK(pl_sd->ks_floodprotect_tick, tick) <= 1 )
		{
...
int mob_delayhide(int tid, unsigned int tick, int id, intptr_t data)
{
struct block_list* bl = map_id2bl(id);
struct mob_data* md = BL_CAST(BL_MOB, bl);
...
if( md )
{
if( md->unhide_timer != tid )
{
ShowError("mob_delayspawn: Timer mismatch: %d != %d\n", tid, md->unhide_timer);
return 0;
}
md->unhide_timer = INVALID_TIMER;
unhide_delay(md);
}
return 0;
}

int unhide_delay(struct mob_data *md){
md->sc.option &= ~OPTION_HIDE;
status_set_viewdata(&md->bl, md->class_);
clif_changeoption(&md->bl);
return 0;
}
...
void mob_damage(struct mob_data *md, struct block_list *src, int damage)
{
	...
	if (battle_config.show_mob_info&11)
