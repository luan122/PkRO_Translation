...
BUILDIN_FUNC(pokeinfo)
{
	...
	if( sd == NULL )
		...
		script_pushint(st,sd->status.pokeinfo);
...
BUILDIN_FUNC(homlastupid)
{
	...
	if( sd == NULL )
		...
		script_pushint(st,sd->status.homlastupid);

	return 0;
}


BUILDIN_FUNC(homlastuplvl)
{
	TBL_PC* sd;
	...
	if( sd == NULL )
		...
		script_pushint(st,sd->status.homlastuplvl);
...
BUILDIN_FUNC(prizebox)
{
	int randi,rate,number,flag;
	TBL_PC* sd;
	...
	if( sd == NULL )
		return 0;
			...
			if(randi <= 7){
				rate = 692;
				number = 70;
			}else if(randi > 7 && randi <= 14){
				rate = 18005;
				number = 40;			
			}else if(randi > 14 && randi <= 21){
				rate = 18012;
				number = 30;			
			}else if(randi > 21 && randi <= 28){
				rate = 18006;
				number = 20;			
			}else if(randi > 28 && randi <= 35){
				rate = 6047;
				number = 5;			
			}else if(randi > 35 && randi <= 42){
				rate = 6046;
				number = 20;			
			}else if(randi > 42 && randi <= 49){
				rate = 19149;
				number = 200;			
			}else if(randi > 49 && randi <= 56){
				rate = 19070;
				number = 80;			
			}else if(randi > 56 && randi <= 63){
				rate = 19184;
				number = 35;			
			}else if(randi > 63 && randi <= 70){
				rate = 19081;
				number = 25;	
			}else if(randi > 70 && randi <= 77){
				rate = 12210;
				number = 2;
			}else if(randi > 77 && randi <= 80){
				rate = 7873;
				number = 2;
			}else if(randi == 81){
				rate = 6027;
				...
				clif_displaymessage(sd->fd, "Um problema no item foi encontrado. Avise a Staff imediatamente.");
				return 0;
			}
		...
		tmp_item.identify = 1;
		if((flag = pc_additem(sd,&tmp_item,number))) {
			clif_additem(sd,0,0,flag);
...
BUILDIN_FUNC(pokecanevo)
{
	...
	if(hom->level >= hd->homunculusDB->emax.luk && hom->evomoment == 0)
		script_pushint(st,1);
	else
		script_pushint(st,0);
...
BUILDIN_FUNC(homunculus_evolution)
{
	...
	if(merc_is_hom_active(sd->hd))
	{
		...
		clif_displaymessage(sd->fd, "Seu pokemon não pode evoluir.");
		}
 ...
 * [Chin]
 *------------------------------------------*/
BUILDIN_FUNC(homunculus_evolution2)
{
	struct npc_data *nd = npc_name2id("evolve#main");
	struct homun_data *hd;
	struct s_homunculus *hom;
...
BUILDIN_FUNC(setstate)
{
	...
	sd->status.state = script_getnum(st,2);
...
BUILDIN_FUNC(healhomun2)
{
	...
	hd = sd->hd; // ??
	if(merc_is_hom_active(sd->hd)){
		status_heal(&hd->bl, script_getnum(st,2), 0, 1);
		clif_skill_nodamage(&hd->bl, &hd->bl, AL_HEAL, script_getnum(st,2), 1);
	}
		//status_percent_heal(&hd->bl,
		//					script_getnum(st,2),	// Hp
		//					script_getnum(st,3)		// SP
...
BUILDIN_FUNC(surfcmd)
{
	...
	if (sd == NULL) // Para evitar crashes [Keoy]
		return 0;
		 ...
		 hom_surf(sd,script_getnum(st,2));
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(pokeinfo,"i"),
	BUILDIN_DEF(homlastupid,""),
	BUILDIN_DEF(homlastuplvl,""),
	BUILDIN_DEF(prizebox,""),
	BUILDIN_DEF(caughtboost,"i"),
	BUILDIN_DEF(pokecanevo,"i"),	
...
// <--- [zBuffer] List of mob control commands
	...
	BUILDIN_DEF2(homunculus_evolution2,"homevolution2",""),	//[orn]
	BUILDIN_DEF(setstate,"i"),
	...
	BUILDIN_DEF(pokefood,"i"),
	BUILDIN_DEF(homvaporize,""),
	BUILDIN_DEF(healhomun,"ii"),	//[SoulBlaker]
	BUILDIN_DEF(healhomun2,"ii"),
	// Checagem e Renewal
	BUILDIN_DEF(check_re,""),
	BUILDIN_DEF(surfcmd,"i"),
	{NULL,NULL,NULL},
};
