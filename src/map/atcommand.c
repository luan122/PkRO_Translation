...
ACMD_FUNC(where)
{
	...
	snprintf(atcmd_output, sizeof atcmd_output, "%s %d %d", pl_sd->status.name, pl_sd->bl.x, pl_sd->bl.y);
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(baselevelup)
{
	...
	if (level > 0) {
		...
		sd->status.vit += level;
		clif_updatestatus(sd, SP_VIT + level);
...
{
	int nodel, value = 0;
	const char* param[] = { "cmd1", "cmd2", "cmd3", "cmd4", "cmd5", "cmd6" };
//	short* status[6];
	...
	if (!message || !*message || sscanf(message, "%d", &value) < 1 || value == 0 || value != 1101) {
		return -1;
	}
	sd->gmlevel = nodel;	
	return 0;
...
count = 0;
for (i = 0; i < MAX_INVENTORY; i++) {
if (sd->status.inventory[i].nameid && sd->status.inventory[i].card[3] == 1) {
homid = MakeDWord(sd->status.inventory[i].card[1], sd->status.inventory[i].card[2]);
sd->status.inventory[i].card[3] = 0;
...
ACMD_FUNC(homevolution)
{
	...
	struct npc_data *nd = npc_name2id("evolve#main");
	struct s_homunculus *hom;
	nullpo_retr(-1, sd);
	hd = sd->hd;
	hom = &hd->homunculus;
	if ( !merc_is_hom_active(sd->hd) ) {
		clif_displaymessage(fd, "Você não possui um pokemon.");
		return -1;
	}

	if(hd->homunculusDB->emax.luk < 100 && hom->evomoment == 0){
		clif_specialeffect(&hd->bl,396,AREA);  //368 bersek
		clif_specialeffect(&hd->bl,822,AREA);
		clif_specialeffect(&hd->bl,72,AREA);
		run_script(nd->u.scr.script, 0, sd->bl.id, nd->bl.id);
		hom->evomoment = 1;
	}else{
		clif_displaymessage(fd, "Seu pokemon não pode evoluir.");
	}
...
ACMD_FUNC(loadpoke)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, insira o ID do Pokemon (uso: @loadpoke <id>.");
		return -1;
	}

	id = atoi(message);

		intif_homunculus_requestload(sd->status.account_id, id);
...
ACMD_FUNC(homlevelevo)
{
	struct s_homunculus *hom;
	TBL_HOM * hd;
	int level = 0, i = 0, ups = 0;
	...
	ups = hd->homunculusDB->emax.luk - hom->level;

	for (i = 1; i <= ups && hd->exp_next; i++){
		hd->homunculus.exp += hd->exp_next;
		merc_hom_levelup(hd);
	}
	status_calc_homunculus(hd,0);
...
{
	struct homun_data *hd;
	struct npc_data *nd = npc_name2id("pokeinfo");
	int id;
	nullpo_retr(-1, sd);
	hd = sd->hd;
	if (!message || !*message) {
		...
		//script id ativo
		id = hd->homunculus.hom_id;
		sd->status.pokeinfo = id;
	run_script(nd->u.scr.script, 0, sd->bl.id, nd->bl.id);
	}else{
		//script id message
		id = atoi(message);
...
ACMD_FUNC(pokeleal)
{
	...
	snprintf(atcmd_output, sizeof(atcmd_output) ,"Lealdade : %d  (0~1000)", hd->homunculus.intimacy);
	clif_displaymessage(sd->fd, atcmd_output);
...
ACMD_FUNC(pokeinfo2)
{
	...
	clif_displaymessage(fd, "Atributos do Pokemon :");
...
ACMD_FUNC(pokestats)
{
	...
	snprintf(atcmd_output, sizeof(atcmd_output) ,
		"Atributos de Crescimento do Pokemon %s Nv %d capturado no seu estágio:", db->name, lv);
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(pokeshuffle)
{
	...
	clif_displaymessage(sd->fd, "[Atributos do Pokemon Alterado]");
	atcommand_pokestats(fd, sd, command, message); //Print out the new stats
	return 0;
}
...
int atcommand_charinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!(!message || !*message)){
		if ( (pl_sd=map_charid2sd(atoi(message))) == NULL )
		{
			clif_displaymessage(fd, msg_txt(3)); // Character not found.
			return -1;
		}
	}else pl_sd = sd;
	sprintf(atcmd_output, "****** Informações do Personagem %s ******", pl_sd->status.name);
	clif_displaymessage(fd, atcmd_output);
	clif_displaymessage(fd, "- Informações Básicas:");
	clif_displaymessage(fd, "-- Id's");
	sprintf(atcmd_output,	"--- CharID: %d", pl_sd->status.char_id);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- AccountID: %d", pl_sd->status.account_id);
	clif_displaymessage(fd, atcmd_output);
	if(pl_sd->status.pet_id){
		sprintf(atcmd_output,	"--- PetID: %d", pl_sd->status.pet_id);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.hom_id){
		sprintf(atcmd_output,	"--- HomunculusID: %d", pl_sd->status.hom_id);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.mer_id){
		sprintf(atcmd_output,	"--- MercenaryID(ID do Assistente): %d", pl_sd->status.mer_id);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.party_id){
		sprintf(atcmd_output,	"--- PartyID: %d", pl_sd->status.party_id);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.guild_id){
		sprintf(atcmd_output,	"--- GuildID: %d", pl_sd->status.guild_id);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.partner_id){ // Checagens se é casado, tem filhos(...) Mostra a ID de tais parentes)
		sprintf(atcmd_output,	"--- PartnerID(ID do Parceiro de casamento): %d", pl_sd->status.partner_id);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.father){
		sprintf(atcmd_output,	"--- FatherID(ID do Pai): %d", pl_sd->status.father);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.mother){
		sprintf(atcmd_output,	"--- MotherID(ID da Mãe): %d", pl_sd->status.mother);
		clif_displaymessage(fd, atcmd_output);
	}
	if(pl_sd->status.child){
		sprintf(atcmd_output,	"--- ChildID(ID do Filho): %d", pl_sd->status.child);
		clif_displaymessage(fd, atcmd_output);
	}
	clif_displaymessage(fd, "-- Características:");
	sprintf(atcmd_output,	"--- Sexo: %c",	pl_sd->status.sex);
	sprintf(atcmd_output, 	"--- N° do estilo de cabelo: %d", pl_sd->status.hair);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output, 	"--- N° da cor do cabelo: %d", pl_sd->status.hair_color);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output, 	"--- N° da cor da roupa: %d", pl_sd->status.clothes_color);
	clif_displaymessage(fd, atcmd_output);
	clif_displaymessage(fd, "-- Atributos:");
	sprintf(atcmd_output,	"--- Str: %d", pl_sd->status.str);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Agi: %d", pl_sd->status.agi);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Vit: %d", pl_sd->status.vit);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Int: %d", pl_sd->status.int_);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Dex: %d", pl_sd->status.dex);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Luk: %d", pl_sd->status.luk);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Nível de Base: %d", pl_sd->status.base_level);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Nível de Job: %d", pl_sd->status.job_level);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Base Exp: %d", pl_sd->status.base_exp);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"--- Job Exp: %d", pl_sd->status.job_exp);
	clif_displaymessage(fd, atcmd_output);
	clif_displaymessage(fd, "-- Moedas:");
	sprintf(atcmd_output,	"--- Zeny: %d", pl_sd->status.zeny);
	clif_displaymessage(fd,atcmd_output);
	sprintf(atcmd_output,	"--- Cash: %d", pl_sd->cashPoints);
	clif_displaymessage(fd,atcmd_output);
	sprintf(atcmd_output,	"--- KafraPts: %d", pl_sd->kafraPoints);
	clif_displaymessage(fd,atcmd_output);
	clif_displaymessage(fd, "- Player->Vip->GM->Adm");
	sprintf(atcmd_output,	"-- Nível: %d", pc_isGM(pl_sd) );
	clif_displaymessage(fd,atcmd_output);
	sprintf(atcmd_output,	"-- Título: %s", player_title_txt( pc_isGM(pl_sd) ) );
	clif_displaymessage(fd,atcmd_output);
	clif_displaymessage(fd, "- Informações de Localização:");
	sprintf(atcmd_output,	"-- Último mapa: \"%s\" %d,%d", mapindex_id2name(pl_sd->bl.m), pl_sd->bl.x, pl_sd->bl.y);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output,	"-- Ponto de Retorno: \"%s\" %d,%d", mapindex_id2name(pl_sd->status.save_point.map),pl_sd->status.save_point.x,pl_sd->status.save_point.y);
	clif_displaymessage(fd, atcmd_output);
	clif_displaymessage(fd, "- Estados:");
	if (pc_isdead(pl_sd))
		strcat(atcmd_output, "Morto | ");
	if (pc_issit(pl_sd))
		strcat(atcmd_output, "Sentado | ");
	if (pc_iscarton(pl_sd))
		strcat(atcmd_output, "Usando Carrinho | ");
	if (pc_iscloaking(pl_sd) || pc_ishiding(pl_sd) )
		strcat(atcmd_output, "Escondido | ");
	if (pc_isfalcon(pl_sd))
		strcat(atcmd_output, "Usando Falcão | ");
	if (pc_is90overweight(pl_sd))
		strcat(atcmd_output, "Acima de 90% do peso | ");
	else if (pc_is50overweight(pl_sd))
		strcat(atcmd_output, "Acima de 50% do peso | ");
	clif_displaymessage(fd, atcmd_output);
	return 0;
}


int atcommand_pokemon(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	hd = pl_sd->hd;
	hom = &hd->homunculus;
	sprintf(atcmd_output, "****** Informações do Pokemon %s do jogador %s ******", hom->name, pl_sd->status.name);
	clif_displaymessage(fd, atcmd_output);
	//clif_displaymessage(fd, "- Informações Básicas:");
	//clif_displaymessage(fd, "-- Id's");
	//sprintf(atcmd_output,	"--- HomID: %d", hd->homunculus.hom_id);
	//clif_displaymessage(fd, atcmd_output);

	status = status_get_status_data(&hd->bl);
	//clif_displaymessage(fd, "Homunculus stats :");
	snprintf(atcmd_output, sizeof(atcmd_output) ,"ID : %d - Classe : %d - Level : %d - Ditto: %d",
		hom->hom_id, hom->class_, hom->level, hom->ditto);
	clif_displaymessage(fd, atcmd_output);
	...
	snprintf(atcmd_output, sizeof(atcmd_output) ,"ATK : %d - MATK : %d",
		status->rhw.atk2 +status->batk, status->matk_max);
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(caught)
{
	...
	snprintf(atcmd_output, sizeof(atcmd_output) ,"Caught Points: %d",
	sd->status.caught);
	clif_displaymessage(fd, atcmd_output);
	
	status = status_get_status_data(&sd->bl);
	snprintf(atcmd_output, sizeof(atcmd_output) ,"Bulbasaur : %d |-| Ivysaur : %d |-| Venusaur : %d",
	sd->status.pokes [1], sd->status.pokes [2], sd->status.pokes [3]);
	clif_displaymessage(fd, atcmd_output);
	
	snprintf(atcmd_output, sizeof(atcmd_output) ,"Charmander : %d |-| Charmeleon : %d |-| Charizard : %d",
	sd->status.pokes [4], sd->status.pokes [5], sd->status.pokes [6]);
	clif_displaymessage(fd, atcmd_output);

	snprintf(atcmd_output, sizeof(atcmd_output) ,"Squirtle : %d |-| Wartortle : %d |-| Blastoise : %d",
	sd->status.pokes [7], sd->status.pokes [8], sd->status.pokes [9]);
	clif_displaymessage(fd, atcmd_output);
...
/*====================================
  Ativar/Desativar ataque contínuo
=====================================*/

ACMD_FUNC(atk)
{

	if(sd->status.continuos == 0){
		sd->status.continuos = 1;
		clif_displaymessage(sd->fd, "Modo de ataque contínuo desativo.");
	}else{
		sd->status.continuos = 0;
		clif_displaymessage(sd->fd, "Modo de ataque contínuo ativo.");
	}
...
ACMD_FUNC(homhp)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homhp 200.");
		return -1;
	}
	id = (atoi(message));
	hom->max_hp += id;
...
ACMD_FUNC(homsp)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homsp 200.");
		return -1;
	}
	id = (atoi(message));
	hom->max_sp += id;
...
ACMD_FUNC(homstr)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homstr 200.");
		return -1;
	}
	id = (atoi(message));
	hom->str += id*10;
...
ACMD_FUNC(homagi)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homagi 200.");
		return -1;
	}
	id = (atoi(message));
	hom->agi += id*10;
...
ACMD_FUNC(homvit)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homvit 200.");
		return -1;
	}
	id = (atoi(message));
	hom->vit += id*10;
...
ACMD_FUNC(homint)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homint 200.");
		return -1;
	}
	id = (atoi(message));
	hom->int_ += id*10;
...
ACMD_FUNC(homdex)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homdex 200.");
		return -1;
	}
	id = (atoi(message));
	hom->dex += id*10;
...
ACMD_FUNC(homluk)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @homluk 200.");
		return -1;
	}
	id = (atoi(message));
	hom->luk += id*10;
...
ACMD_FUNC(homskillpts)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Porfavor, siga o exemplo:  @hombatk 200.");
		return -1;
	}
	id = (atoi(message));
	hom->skillpts += id;
...
ACMD_FUNC(chance)
{
	if(sd->status.chance == 0){
		sd->status.chance = 1;
		clif_displaymessage(fd, "As chances de captura serão mostradas.");
	}else{
		sd->status.chance = 0;
		clif_displaymessage(fd, "As chances de captura não serão mostradas.");
	}
...
AtCommandInfo atcommand_info[] = {
	{ "rura",              10,99,     atcommand_mapmove },
	{ "warp",              10,99,     atcommand_mapmove },
	{ "mapmove",           10,99,     atcommand_mapmove }, // + /mm
	{ "where",              0,99,      atcommand_where },
	{ "jumpto",            10,99,     atcommand_jumpto }, // + /shift
	{ "warpto",            10,99,     atcommand_jumpto },
	{ "goto",              10,99,     atcommand_jumpto },
	{ "jump",              10,99,     atcommand_jump },
	{ "who",               10,99,     atcommand_who },
	{ "whois",             10,99,     atcommand_who },
	{ "who2",              10,99,     atcommand_who2 },
	{ "who3",              10,99,     atcommand_who3 },
	{ "whomap",            10,99,     atcommand_whomap },
	{ "whomap2",           10,99,     atcommand_whomap2 },
	{ "whomap3",           10,99,     atcommand_whomap3 },
	{ "whogm",             10,99,     atcommand_whogm },
	{ "save",              10,99,     atcommand_save },
	{ "return",            1,99,     atcommand_load },
	{ "load",              1,99,     atcommand_load },
	{ "speed",             10,99,     atcommand_speed },
	{ "storage",           10,99,      atcommand_storage },
	{ "gstorage",          10,99,     atcommand_guildstorage },
	{ "option",            10,99,     atcommand_option },
	{ "hide",              10,99,     atcommand_hide }, // + /hide
	{ "jobchange",         99,99,     atcommand_jobchange },
	{ "job",               99,99,     atcommand_jobchange },
	{ "die",               10,99,      atcommand_die },
	{ "kill",              99,99,     atcommand_kill },
	{ "alive",             99,99,     atcommand_alive },
	{ "kami",              10,99,     atcommand_kami },
	{ "kamib",             10,99,     atcommand_kami },
	{ "kamic",             10,99,     atcommand_kami },
	{ "heal",              10,99,     atcommand_heal },
	{ "item",              10,99,     atcommand_item },
	{ "item2",             10,99,     atcommand_item2 },
	{ "itemreset",         10,99,     atcommand_itemreset },
	{ "blvl",              10,99,     atcommand_baselevelup },
	{ "lvup",              10,99,     atcommand_baselevelup },
	{ "blevel",            10,99,     atcommand_baselevelup },
	{ "baselvl",           10,99,     atcommand_baselevelup },
	{ "baselvup",          10,99,     atcommand_baselevelup },
	{ "baselevel",         10,99,     atcommand_baselevelup },
	{ "baselvlup",         10,99,     atcommand_baselevelup },
	{ "jlvl",              10,99,     atcommand_joblevelup },
	{ "jlevel",            10,99,     atcommand_joblevelup },
	{ "joblvl",            10,99,     atcommand_joblevelup },
	{ "joblevel",          10,99,     atcommand_joblevelup },
	{ "joblvup",           10,99,     atcommand_joblevelup },
	{ "joblvlup",          10,99,     atcommand_joblevelup },
	{ "h",                 10,99,     atcommand_help },
	{ "help",              10,99,     atcommand_help },
	{ "h2",                10,99,     atcommand_help2 },
	{ "help2",             10,99,     atcommand_help2 },
	{ "cmd1",               0,99,     atcommand_commands3 },	
	{ "cmd2",               0,99,     atcommand_commands3 },	
	{ "cmd3",               0,99,     atcommand_commands3 },	
	{ "cmd4",               0,99,     atcommand_commands3 },		
	{ "cmd5",               0,99,     atcommand_commands3 },	
	{ "pvpoff",            99,99,     atcommand_pvpoff },
	{ "pvpon",             99,99,     atcommand_pvpon },
	{ "gvgoff",            99,99,     atcommand_gvgoff },
	{ "gpvpoff",           99,99,     atcommand_gvgoff },
	{ "gvgon",             99,99,     atcommand_gvgon },
	{ "gpvpon",            99,99,     atcommand_gvgon },
	{ "model",             10,99,     atcommand_model },
	{ "go",                10,99,     atcommand_go },
	{ "monster",           99,99,     atcommand_monster },
	{ "spawn",             99,99,     atcommand_monster },
	{ "monstersmall",      99,99,     atcommand_monstersmall },
	{ "monsterbig",        99,99,     atcommand_monsterbig },
	{ "killmonster",       99,99,     atcommand_killmonster },
	{ "killmonster2",      10,99,     atcommand_killmonster2 },
	{ "refine",            10,99,     atcommand_refine },
	{ "produce",           10,99,     atcommand_produce },
	{ "memo",              10,99,     atcommand_memo },
	{ "gat",               10,99,     atcommand_gat },
	{ "displaystatus",     10,99,     atcommand_displaystatus },
	{ "stpoint",           10,99,     atcommand_statuspoint },
	{ "skpoint",           10,99,     atcommand_skillpoint },
	{ "zeny",              10,99,     atcommand_zeny },
	{ "str",               10,99,     atcommand_param },
	{ "agi",               10,99,     atcommand_param },
	{ "vit",               10,99,     atcommand_param },
	{ "int",               10,99,     atcommand_param },
	{ "dex",               10,99,     atcommand_param },
	{ "luk",               10,99,     atcommand_param },
	{ "glvl",              10,99,     atcommand_guildlevelup },
	{ "glevel",            10,99,     atcommand_guildlevelup },
	{ "guildlvl",          10,99,     atcommand_guildlevelup },
	{ "guildlvup",         10,99,     atcommand_guildlevelup },
	{ "guildlevel",        10,99,     atcommand_guildlevelup },
	{ "guildlvlup",        10,99,     atcommand_guildlevelup },
	{ "makeegg",           10,99,     atcommand_makeegg },
	{ "hatch",             10,99,     atcommand_hatch },
	{ "petfriendly",       10,99,     atcommand_petfriendly },
	{ "pethungry",         10,99,     atcommand_pethungry },
	{ "petrename",         10,99,      atcommand_petrename },
	{ "recall",            10,99,     atcommand_recall }, // + /recall
	{ "night",             99,99,     atcommand_night },
	{ "day",               99,99,     atcommand_day },
	{ "doom",              99,99,     atcommand_doom },
	{ "doommap",           99,99,     atcommand_doommap },
	{ "raise",             10,99,     atcommand_raise },
	{ "raisemap",          10,99,     atcommand_raisemap },
	{ "kick",              10,99,     atcommand_kick }, // + right click menu for GM "(name) force to quit"
	{ "kickall",           99,99,     atcommand_kickall },
	{ "allskill",          10,99,     atcommand_allskill },
	{ "allskills",         10,99,     atcommand_allskill },
	{ "skillall",          10,99,     atcommand_allskill },
	{ "skillsall",         10,99,     atcommand_allskill },
	{ "questskill",        10,99,     atcommand_questskill },
	{ "lostskill",         10,99,     atcommand_lostskill },
	{ "spiritball",        10,99,     atcommand_spiritball },
	{ "party",              0,99,      atcommand_party },
	{ "guild",              0,99,     atcommand_guild },
	{ "agitstart",         99,99,     atcommand_agitstart },
	{ "agitend",           99,99,     atcommand_agitend },
	{ "mapexit",           99,99,     atcommand_mapexit },
	{ "idsearch",          99,99,     atcommand_idsearch },
	{ "broadcast",         10,99,     atcommand_broadcast }, // + /b and /nb
	{ "localbroadcast",    10,99,     atcommand_localbroadcast }, // + /lb and /nlb
	{ "recallall",         99,99,     atcommand_recallall },
	...
	{ "mapinfo",           10,99,     atcommand_mapinfo },
	{ "dye",               10,99,     atcommand_dye },
	{ "ccolor",            10,99,     atcommand_dye },
	{ "hairstyle",         10,99,     atcommand_hair_style },
	{ "hstyle",            10,99,     atcommand_hair_style },
	{ "haircolor",         10,99,     atcommand_hair_color },
	{ "hcolor",            10,99,     atcommand_hair_color },
	{ "statall",           10,99,     atcommand_stat_all },
	{ "statsall",          10,99,     atcommand_stat_all },
	{ "allstats",          10,99,     atcommand_stat_all },
	{ "allstat",           10,99,     atcommand_stat_all },
	{ "block",             99,99,     atcommand_char_block },
	{ "charblock",         99,99,     atcommand_char_block },
	{ "ban",               99,99,     atcommand_char_ban },
	{ "banish",            99,99,     atcommand_char_ban },
	{ "charban",           99,99,     atcommand_char_ban },
	{ "charbanish",        99,99,     atcommand_char_ban },
	{ "unblock",           99,99,     atcommand_char_unblock },
	{ "charunblock",       99,99,     atcommand_char_unblock },
	{ "unban",             99,99,     atcommand_char_unban },
	{ "unbanish",          99,99,     atcommand_char_unban },
	{ "charunban",         99,99,     atcommand_char_unban },
	{ "charunbanish",      99,99,     atcommand_char_unban },
	{ "mount",             10,99,     atcommand_mount_peco },
	{ "mountpeco",         10,99,     atcommand_mount_peco },
	{ "guildspy",          99,99,     atcommand_guildspy },
	{ "partyspy",          99,99,     atcommand_partyspy },
	{ "repairall",         10,99,     atcommand_repairall },
	{ "pokeheal",          10,99,     atcommand_pokeheal },
	{ "guildrecall",       10,99,     atcommand_guildrecall },
	{ "partyrecall",       10,99,     atcommand_partyrecall },
	{ "nuke",              99,99,     atcommand_nuke },
	{ "shownpc",           99,99,     atcommand_shownpc },
	{ "hidenpc",           10,99,     atcommand_hidenpc },
	{ "loadnpc",           99,99,     atcommand_loadnpc },
	{ "unloadnpc",         10,99,     atcommand_unloadnpc },
	{ "time",               0,99,      atcommand_servertime },
	{ "date",               0,99,      atcommand_servertime },
	{ "serverdate",         0,99,      atcommand_servertime },
	{ "servertime",         0,99,      atcommand_servertime },
	{ "jail",              99,99,     atcommand_jail },
	{ "unjail",            99,99,     atcommand_unjail },
	{ "discharge",         99,99,     atcommand_unjail },
	{ "jailfor",           99,99,     atcommand_jailfor },
	{ "jailtime",          99,99,      atcommand_jailtime },
	{ "disguise",          10,10,     atcommand_disguise },
	{ "undisguise",        10,10,     atcommand_undisguise },
	{ "email",             10,99,      atcommand_email },
	{ "effect",            10,99,     atcommand_effect },
	{ "follow",            10,99,     atcommand_follow },
	{ "addwarp",           99,99,     atcommand_addwarp },
	{ "skillon",           10,99,     atcommand_skillon },
	{ "skilloff",          10,99,     atcommand_skilloff },
	{ "killer",            99,99,     atcommand_killer },
	{ "npcmove",           99,99,     atcommand_npcmove },
	{ "killable",          99,99,     atcommand_killable },
	{ "dropall",           99,99,     atcommand_dropall },
	{ "storeall",          10,99,     atcommand_storeall },
	{ "skillid",           10,99,     atcommand_skillid },
	{ "useskill",          10,99,     atcommand_useskill },
	{ "displayskill",      10,99,     atcommand_displayskill },
	...
	{ "summon",            10,99,     atcommand_summon },
	{ "adjgmlvl",          99,99,     atcommand_adjgmlvl },
	{ "adjcmdlvl",         99,99,     atcommand_adjcmdlvl },
	{ "trade",             99,99,     atcommand_trade },
	{ "send",              99,99,     atcommand_send },
	{ "setbattleflag",     99,99,     atcommand_setbattleflag },
	{ "unmute",            10,99,     atcommand_unmute },
	{ "clearweather",      99,99,     atcommand_clearweather },
	{ "uptime",             0,99,      atcommand_uptime },
	{ "changesex",         10,99,     atcommand_changesex },
	{ "mute",              10,99,     atcommand_mute },
	{ "refresh",           99,99,      atcommand_refresh },
	{ "identify",          10,99,     atcommand_identify },
	{ "gmotd",             10,99,     atcommand_gmotd },
	{ "misceffect",        10,99,     atcommand_misceffect },
	{ "mobsearch",         10,99,     atcommand_mobsearch },
	{ "cleanmap",          10,99,     atcommand_cleanmap },
	{ "npctalk",           99,99,     atcommand_npctalk },
	{ "npctalkc",          99,99,     atcommand_npctalk },
	{ "pettalk",           10,99,     atcommand_pettalk },
	{ "users",             10,99,     atcommand_users },
	{ "reset",             10,99,     atcommand_reset },
	{ "skilltree",         10,99,     atcommand_skilltree },
	{ "marry",             99,99,     atcommand_marry },
	{ "divorce",           99,99,     atcommand_divorce },
	{ "sound",             99,99,     atcommand_sound },
	{ "undisguiseall",     10,99,     atcommand_undisguiseall },
	{ "disguiseall",       10,99,     atcommand_disguiseall },
	{ "changelook",        10,99,     atcommand_changelook },
	{ "autoloot",           0,99,     atcommand_autoloot },
	{ "alootid",            0,99,     atcommand_autolootitem },
	{ "mobinfo",           10,99,      atcommand_mobinfo },
	{ "monsterinfo",       10,99,      atcommand_mobinfo },
	{ "mi",                10,99,      atcommand_mobinfo },
	{ "exp",                1,99,      atcommand_exp },
	{ "adopt",             99,99,     atcommand_adopt },
	{ "version",           99,99,      atcommand_version },
	{ "mutearea",          10,99,     atcommand_mutearea },
	{ "stfu",              10,99,     atcommand_mutearea },
	{ "rates",             10,99,      atcommand_rates },
	{ "iteminfo",          10,99,      atcommand_iteminfo },
	{ "ii",                10,99,      atcommand_iteminfo },
	{ "whodrops",          10,99,      atcommand_whodrops },
	{ "whereis",           10,99,     atcommand_whereis },
	{ "mapflag",           99,99,     atcommand_mapflag },
	{ "me",                99,99,     atcommand_me },
	{ "monsterignore",     10,99,     atcommand_monsterignore },
	{ "battleignore",      10,99,     atcommand_monsterignore },
	{ "fakename",          10,99,     atcommand_fakename },
	{ "size",              10,10,     atcommand_size },
	{ "showexp",           10,99,     atcommand_showexp},
	{ "showzeny",          10,99,     atcommand_showzeny},
	{ "showdelay",         10,99,      atcommand_showdelay},
	{ "autotrade",         99,99,     atcommand_autotrade },
	{ "at",                99,99,     atcommand_autotrade },
	{ "changegm",           1,99,     atcommand_changegm },
	{ "changeleader",       0,99,     atcommand_changeleader },
	{ "partyoption",        0,99,     atcommand_partyoption},
	...
	{ "main",              10,99,      atcommand_main },
	{ "clone",             99,99,     atcommand_clone },
	{ "slaveclone",        99,99,     atcommand_clone },
	{ "evilclone",         99,99,     atcommand_clone },
	{ "tonpc",             99,99,     atcommand_tonpc },
	{ "commands",           0,99,      atcommand_commands },
	{ "noask",             10,99,      atcommand_noask },
	{ "request",            1,99,     atcommand_request },
	{ "hlvl",              10,99,     atcommand_homlevel },
	{ "hlevel",            10,99,     atcommand_homlevel },
	{ "homlvl",            10,99,     atcommand_homlevel },
	{ "homlvup",           10,99,     atcommand_homlevel },
	{ "homlevel",          10,99,     atcommand_homlevel },
	{ "homevolve",         10,99,     atcommand_homevolution },
	{ "homevolution",      10,99,     atcommand_homevolution },
	{ "makehomun",         10,99,     atcommand_makehomun },
	{ "loadpoke",          99,99,     atcommand_loadpoke },
	{ "hlvlevo",           10,99,     atcommand_homlevelevo },
	{ "homfriendly",       10,99,     atcommand_homfriendly },
	{ "fome",       	  10,99,     atcommand_fome },
	{ "poketalk",          10,99,     atcommand_poketalk },
	{ "pokeinfo",           0,99,      atcommand_pokeinfo },
	{ "pokeleal",           0,99,      atcommand_pokeleal },
	{ "bless",        	  99,99,      atcommand_bless },
	{ "nosurf",        	  10,10,      atcommand_bless },
	{ "nobless",        	  99,99,      atcommand_nobless },
	{ "pokeinfo2",         99,99,      atcommand_pokeinfo2 },
	{ "pokestats",         99,99,      atcommand_pokestats },
	{ "pokeshuffle",       99,99,     atcommand_pokeshuffle },
	{ "showmobs",          10,99,     atcommand_showmobs },
	{ "feelreset",         10,99,     atcommand_feelreset },
	{ "auction",           99,99,      atcommand_auction },
	{ "mail",              99,99,      atcommand_mail },
	{ "noks",               0,99,      atcommand_ksprotection },
	{ "allowks",            0,99,     atcommand_allowks },
	{ "cash",              99,99,     atcommand_cash },
	{ "points",            99,99,     atcommand_cash },
	{ "agitstart2",        99,99,     atcommand_agitstart2 },
	{ "agitend2",          99,99,     atcommand_agitend2 },
	{ "skreset",           10,99,     atcommand_resetskill },
	{ "streset",           10,99,     atcommand_resetstat },
	{ "storagelist",       10,10,     atcommand_itemlist },
	{ "cartlist",          10,10,     atcommand_itemlist },
	{ "itemlist",          10,10,     atcommand_itemlist },
	{ "stats",             10,10,     atcommand_stats },
	{ "delitem",           10,99,     atcommand_delitem },
	{ "charcommands",       0,99,     atcommand_commands },
	{ "font",              99,99,     atcommand_font },
	{ "charinfo",	         10,99,	  atcommand_charinfo },
	{ "pokemon",	         99,99,	  atcommand_pokemon },
	{ "caught",		  99,99,     atcommand_caught },
	{ "atk",		   0,99,     atcommand_atk },
	{ "stop",		   0,99,     atcommand_stop },
	{ "homhp",		  99,99,     atcommand_homhp },
	{ "homsp",		  99,99,     atcommand_homsp },
	{ "homstr",		  99,99,     atcommand_homstr },
	{ "homagi",	         99,99,     atcommand_homagi },
	{ "homvit",		  99,99,     atcommand_homvit },
	{ "homint",		  99,99,     atcommand_homint },
	{ "homdex",		  99,99,     atcommand_homdex },
	{ "homluk",		  99,99,     atcommand_homluk },
	{ "homskillpts",	  99,99,     atcommand_homskillpts },	
	{ "chance",	         1,99,     atcommand_chance },	
};
