...
void vending_vendinglistreq(struct map_session_data* sd, int id)
{
	...
	for( i = 0; i < count; i++ )
	{
		if(vsd->status.cart[i].nameid >= 8031 && vsd->status.cart[i].nameid <= 8181){
			snprintf(atcmd_output, sizeof(atcmd_output) ,"Homunculu ID: %d ", MakeDWord(vsd->status.cart[i].card[1], vsd->status.cart[i].card[2]));
			clif_displaymessage(sd->fd, atcmd_output);
		}	
	}

		clif_displaymessage(sd->fd, "OBS: Os IDs acima podem não estar na ordem que aparecem na loja.");
		clif_displaymessage(sd->fd, "OBS: Antes de comprar, utilize o @pokeinfo ID para se certificar que está fazendo a compra certa.");
